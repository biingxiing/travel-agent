...
export const generatePlanTool: SubagentTool = {
  name: 'call_generator',
  description: 'Create the initial multi-day travel itinerary from the TripBrief and real-world prefetch data. Only for first-time plan creation. Requires call_extractor and call_prefetch to have run first. Reads brief and prefetch data from session automatically — invoke with no arguments. Streams the itinerary to the client as it generates. If this tool returns a "Tool error: LLM stream idle" message, retry call_generator immediately — do NOT call call_prefetch again. The prefetch data is already in session.',
  parametersSchema: {
    type: 'object',
    properties: {
