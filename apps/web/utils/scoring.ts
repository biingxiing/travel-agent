...
function gradeFromScore(score: number | null): Grade {
...
// Handle both web type ('desc') and actual runtime field ('description'); tips can be string or string[]
function itemText(item: PlanItem): string {
  const parts: string[] = [item.title]
  const anyItem = item as Record<string, unknown>
  const desc = (anyItem.description as string) || item.desc || ''
  if (desc) parts.push(desc)
  const tips = anyItem.tips
  if (Array.isArray(tips)) parts.push(...(tips as string[]))
  else if (typeof tips === 'string' && tips) parts.push(tips)
  return parts.join(' ')
}
...
function scoreTransportItem(item: PlanItem): ItemScore {
  ...
  const checks: ScoreCheck[] = [
    chk(
      '航班 / 车次号',
      anyMatch(t, [
        /\b[A-Z]{2}\d{3,4}\b/, // CA1234, MU5678, CZ3456
        /\b[GDZKTYJ]\d{2,4}次?\b/i, // G1234, K456次, D12
        /航班号|车次|班次/,
      ]),
      35,
    ),
    ...
    chk(
      '预订 / 出行提示',
      (Array.isArray(anyItem.tips) ? (anyItem.tips as string[]).length > 0 : !!anyItem.tips) ||
...
function scoreLodgingItem(item: PlanItem): ItemScore {
  ...
  const checks: ScoreCheck[] = [
    ...
    chk(
      '设施 / 须知',
      anyMatch(t, [/含早餐|免费早餐|早餐/, /停车|WiFi|泳池|健身|SPA/i, /预订|取消政策|订房须知/]),
      10,
    ),
  ]
  const score = checks.reduce((s, c) => s + c.points, 0)
...
function scoreAttractionItem(item: PlanItem): ItemScore {
  const t = itemText(item)
  const anyItem = item as Record<string, unknown>
  const descLen = ((anyItem.description as string) || item.desc || '').length
  const checks: ScoreCheck[] = [
    chk(
      '开放时间',
      anyMatch(t, [
        /开放时间|营业时间|开门时间/,
        /全天开放|24小时开放/,
        /\d{1,2}:\d{2}\s*[-~至到]\s*\d{1,2}:\d{2}/, // "09:00-17:00"
        /每(天|日)\s*\d{1,2}:\d{2}/,
      ]),
      25,
    ),
...
function scoreMealItem(item: PlanItem): ItemScore {
  ...
  const checks: ScoreCheck[] = [
    ...
    chk(
      '餐厅位置',
      !!(anyItem.location) || anyMatch(t, [/路|街|区.{0,2}号/, /位于|地址|靠近/]),
      20,
    ),
...
export function scorePlan(plan: Plan): ItineraryScore {
  ...
  // Coverage: per-day completeness
  const totalDays = plan.days
  const daysWithTransport = plan.dailyPlans.filter((d) =>
    d.items.some((i) => i.type === 'transport'),
  ).length
  const daysWithLodging = plan.dailyPlans.filter((d) =>
    d.items.some((i) => i.type === 'lodging'),
  ).length
  const daysWithAttractions = plan.dailyPlans.filter(
    (d) => d.items.filter((i) => i.type === 'attraction' || i.type === 'activity').length >= 2,
  ).length
  ...
  // Overall: category weights redistribute to coverage when category is absent
  ...
  for (const { score, weight } of catWeights) {
    if (score !== null) {
      ...
      coverageWeight += weight // absent category weight goes to coverage
    }
  }
  ...
  if (transport.score === null) {
    suggestions.push('行程中未包含交通安排，建议添加航班或火车具体信息')
  } else if (transport.score < 60) {
    suggestions.push('交通信息较简略，建议补充具体航班 / 车次号、舱位类型和乘车站点')
  }
  if (lodging.score === null) {
    suggestions.push('行程中未包含住宿安排，建议添加酒店名称和具体房型')
  } else if (lodging.score < 60) {
    suggestions.push('住宿信息较简略，建议补充具体房型、入住退房时间及每晚价格')
  }
  if (attraction.score === null) {
    suggestions.push('行程中未检测到景点活动，建议添加具体游览项目')
  } else if (attraction.score < 60) {
    suggestions.push('景点信息较简略，建议补充开放时间、门票价格和建议游览时长')
  }
